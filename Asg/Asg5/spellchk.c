// $Id: spellchk.c,v 1.33 2012-12-08 03:18:43-08 - - $
...
//      char* input[72];
//      int none = scanf("%74s",input);
      if(yytext == EOF) break;
      bool its = has_hashset(hashset, yytext);
      if(its != true){
         int *text = (int)yytext;
         tolower(text);
         bool it_is = has_hashset(hashset, yytext);
         if(it_is != true){
            printf("Yu missspealled %s!\n", yytext);
         }
      }
